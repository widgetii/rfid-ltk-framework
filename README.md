...
- `app` содержит приложений. Каждая поддиректория - это директория приложения.
- `build` содержит скрипты сборки и настройки сборки.
...
Каждый компонент также можно собирать по отдельности, вызвав сборку в его
директории. При этом будут (пере-) собраны все его зависимости.

Каждое приложение необходимо собирать по отдельности, вызвав сборку в его
директории. При этом все необходимые ему компоненты будут (пере-) собраны.
